...
class TypeRacerOnly(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if 't!tg train' in message_content_lower:
            ...
            if message.author.id == 476016555981930526:  # pasta's Discord ID
                ...
                embed.set_thumbnail(
                    url='https://cdn.discordapp.com/attachments/448997775309537280/791689215531679764/pet.png')
                ...
                embed.set_footer(
                    text='Copied from Tatsu#8792 to meme pasta | Wrapped Tatsugotchis are tradable, but not unwrapped ones.')
    ...
    async def keegan(self, ctx, *args):
        ...
        if len(args) == 0:
            file_ = discord.File(
                TYPERACER_RECORDS_JSON, filename=f"typeracer_records_{self.last_updated.lower()}.json")
    ...
    async def construct_embeds(self):
        ...
        for member, url in three_hundred_information.items():
            ...
            wpm = round(12000 * (result['length'] - 1) /
        ...
        all_time_leaders = [
            user for category in self.races_alltime.values() for user in category]
        all_time_leaders += [user for category in self.points_alltime.values()
                             for user in category]
        all_time_leaders += [user for category in self.awards_alltime.values()
                             for user in category]
        ...
        for user in all_time_leaders:
            ...
            try:
                ...
                first_race_timestamp = c.execute(
                    f"SELECT * FROM t_{user} ORDER BY t ASC LIMIT 1").fetchone()[1]
                time_difference = right_now - first_race_timestamp
                points_time_difference = min(
                    right_now - 1_501_113_600, time_difference)
        ...
        def helper_sorter(param): return {k: v[param]
                                          for k, v in all_time_data.items()}

        def helper_flag(x): return f"{self.get_flag(x)} {x}"
        def helper_round(x): return f'{round(x):,}'
        ...
        for key, value in all_time_data.items():
            ...
            races_daily_average_dict.update({
                key: f""" ({f"{round(value['races'], 2):,}"} races over {f"{days:,}"} days)"""
            })
            ...
            chars_typed_metadata_dict.update({
                key: f""" ({f"{round(value['chars_typed'] / value['races'], 2):,}"} average chars over {f"{value['races']:,}"} races)"""
            })

            medal_breakdown_dict.update({
                key: f""" (:first_place: {f"{medal_breakdown[1]:,}"} :second_place: {f"{medal_breakdown[2]:,}"} :third_place: {f"{medal_breakdown[3]:,}"})"""
            })

        races_embed.add_field(name='All-Time Leaders',
                              value=helper_formatter(helper_sorter(
                                  'races'), helper_flag, helper_round, (True,)),
                              inline=False)
        races_embed.add_field(name='Highest Average Daily Races',
                              value=helper_formatter(helper_sorter(
                                  'races_daily_average'), helper_flag, helper_round, (True, races_daily_average_dict)),
                              inline=False)
        races_embed.add_field(name='Most Characters Typed',
                              value=helper_formatter(helper_sorter(
                                  'chars_typed'), helper_flag, helper_round, (True, chars_typed_metadata_dict)),
                              inline=False)
        races_embed.add_field(name='Most Time Spent Racing',
                              value=helper_formatter(helper_sorter(
                                  'seconds_played'), helper_flag, seconds_to_text, (True,)),
                              inline=False)
        races_embed.add_field(name='Highest Average Daily Time Spent Racing',
                              value=helper_formatter(helper_sorter(
                                  'seconds_played_daily_average'), helper_flag, seconds_to_text, (True,)),
                              inline=False)
        ...
        points_embed.add_field(name='All-Time Leaders',
                               value=helper_formatter(helper_sorter(
                                   'points'), helper_flag, helper_round, (True,)),
                               inline=False)
        points_embed.add_field(name='Highest Average Daily Points',
                               value=helper_formatter(helper_sorter(
                                   'points_daily_average'), helper_flag, helper_round, (True, points_daily_average_dict)),
                               inline=False)
        points_embed.add_field(name='All-Time Total Points (Includes Retroactive Points)',
                               value=helper_formatter(helper_sorter(
                                   'total_points'), helper_flag, helper_round, (True,)),
                               inline=False)
        points_embed.set_footer(
            text='Retroactive points represent the total number of points a user would have gained, before points were introduced in 2017')
        ...
        medal_tally = {k: sum(v['medals'].values())
                       for k, v in all_time_data.items()}

        awards_embed.add_field(name='All-Time Leaders',
                               value=helper_formatter(
                                   medal_tally, helper_flag, helper_round, (True, medal_breakdown_dict)),
                               inline=False)

        records_held_embed = discord.Embed(title='Records Held',
                                           color=discord.Color(records_held_color))
        ...
        records_held_embed.add_field(name='Top Users',
                                     value=helper_formatter(
                                         self.user_tally, helper_flag, helper_round, (False,)),
                                     inline=False)
    ...
    def record_field_constructor(self, record_information, unit):
        ...
        if 'Race' in record_information['title'].split(' '):
            emote = ':cinema:'
        else:
            emote = TR_INFO
        try:
