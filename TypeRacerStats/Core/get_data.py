...
class GetData(commands.Cog):
    ...
    async def today(self, ctx, *args):
        ...
        if len(args) == 2:
            try:
                today_timestamp_temp = (datetime.datetime.strptime(
        ...
        date = datetime.datetime.fromtimestamp(
            today_timestamp).strftime('%B %-d, %Y')
        ...
        if data:
            c.executemany(
                f"INSERT INTO {file_name} VALUES (?, ?, ?, ?, ?)", data)
    ...
    async def week(self, ctx, *args):
        ...
        if len(args) == 2:
            try:
                ...
                today_temp = datetime.datetime.strptime(
                    args[1], parse_string).date()
                if today_temp != today:
                    same_day = False
                if today_temp > today:
                    await ctx.send(content=f"<@{user_id}>",
        ...
        for row in data:
            ...
            wpm_total += wpm
            if not wpm_best or wpm_best < wpm:
                wpm_best = wpm
            if not wpm_worst or wpm_worst > wpm:
                wpm_worst = wpm
