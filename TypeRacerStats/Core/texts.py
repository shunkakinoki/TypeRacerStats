        ...
        with open(TEXTS_FILE_PATH_CSV, 'r') as csvfile:
            ...
            for row in reader:
                ...
                if query_length >= len(text):
                    texts.append([row[0], text, row[2], Levenshtein.distance(query, text), 0])
                else:
                    min_index = 0
                    min_distance = 10000000
                    for i in range(0, len(text) - query_length):
                        cur_distance = Levenshtein.distance(query, text[i:i + query_length])
                        ...
                        if cur_distance == 0:
                            ...
                            min_index = i
                            break
                    texts.append([row[0], text, row[2], min_distance, min_index])
