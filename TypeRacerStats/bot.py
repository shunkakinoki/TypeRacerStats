...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Game(
        name=f"{DEFAULT_COMMAND_PREFIX}help | by e6f4e37l#0785 and keegan#1689"
    ))
...
async def on_guild_join(guild):
    ...
    for guild in guilds:
        try:
            ...
            pass
    await eugene.send(embed=discord.Embed(
...
async def on_command(ctx):
    ...
    try:
        user_data = c.execute(
            f"SELECT * FROM {TABLE_KEY} WHERE id = ? LIMIT 1",
            (user_id, )).fetchall()
        if not user_data:
            embed = discord.Embed(
                title='Hello!',
                color=discord.Color(0),
                description=(
                    "You can get started by using the command `-help`\n\n"
...
if __name__ == '__main__':
    for filename in os.listdir('TypeRacerStats/Core'):
        if filename.endswith(
                '.py'
        ) and filename != '__init__.py' and filename != 'christmas_2020.py':
